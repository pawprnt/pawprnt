...
function updateLyrics(lyricsBox, lines, elapsed) {
  ...
  if (!lyricsBox.querySelector(".st-lyrics-inner")) {
    lyricsBox.innerHTML = '<div class="st-badge">lyrics</div><div class="st-lyrics-inner">' +
      lines.map((l, i) => '<div class="st-lyric-line" data-i="' + i + '">' + stEsc(l.text) + "</div>").join("") +
      "</div>";
  }
...
function initStatus(container) {
  ...
  lyricsBox.className = "st-card st-lyrics";
  lyricsBox.style.display = "none";
  right.appendChild(lyricsBox);
  ...
  function activityCard(a) {
    ...
    if (hasTime) {
      const tick = () => {
        const now = Date.now();
        const total = a.timestamps.end - a.timestamps.start;
        const elapsed = now - a.timestamps.start;
  ...
  function linksCard() {
    ...
    const socials = [
      ...
      ["x", "Twitter", "https://x.com/foxinwinter"],
    ...
    const li = (i) => {
      ...
      return '<li><a href="' + i[2] + '" target="_blank" rel="noopener">' +
        '<svg class="st-ico" viewBox="' + ico.vb + '" fill="' + (ico.fill || "currentColor") + '" aria-hidden="true">' + ico.body + "</svg>" +
        i[1] + "</a></li>";
    };
    card.innerHTML =
  ...
  function renderRight(d, lf) {
    ...
    if (music) {
      ...
      if (lastSong.type === "lf") {
        rightMusic.appendChild(lfCard(lastSong.data));
      } else if (lastSong.type === "spotify") {
  ...
  function render() {
    ...
    let songDuration = 0;
    if (music) {
      songId = (music === (lanyard || {}).spotify ? "spotify:" : "act:") + (music.song || music.details || "") + ":" + (music.artist || music.state || "");
